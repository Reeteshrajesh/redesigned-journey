...
export async function generateMetadata({
  ...
  const articleTags = article.related_tags
    ? article.related_tags
        ...
        .filter((tag) => tag.trim())
        .slice(0, 10)
    : ([category, article.news_type, article.company_name].filter(
        Boolean,
      ) as string[]);
  ...
  return {
    ...
    other: {
      ...
      "article:author":
        article.source || article.author || "welomoney Editorial Team",
      ...
      "twitter:data2": new Date(article.created_at).toLocaleDateString(
        "en-US",
        {
          month: "short",
          day: "numeric",
...
export default async function ArticlePage({ params }: ArticlePageProps) {
  ...
  const relatedArticles = allArticles
    .filter(
      (a) =>
        a.id !== article.id &&
        (a.company_name === article.company_name ||
